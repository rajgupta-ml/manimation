...
import { ChevronDown, ChevronRight, Eye, EyeOff, Plus, Video, Volume2, VolumeX } from "lucide-react"
...
const Timeline = ({ selectedScene, onSceneSelect, isCollapsed, onToggleCollapse }: TimelineProps) => {
  ...
  const toggleTrackVisibility = (trackId: string) => {
    setTracks(tracks.map((track) => (track.id === trackId ? { ...track, isVisible: !track.isVisible } : track)))
  }

  const toggleTrackMute = (trackId: string) => {
    setTracks(
      tracks.map((track) =>
        track.id === trackId && track.type === "audio" ? { ...track, isMuted: !track.isMuted } : track,
      ),
    )
  }
    ...
    <div className="text-foreground">
      <div className="flex items-center justify-between p-3 border-b ">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="ghost" size="sm" onClick={onToggleCollapse} className="">
      ...
      <div className="p-0">
        <div className="flex">
          ...
          <div className="w-48 ">
            ...
            {tracks.map((track) => (
              <div key={track.id} className="h-20 border-b flex items-center px-4">
          ...
          <div className="flex-1 overflow-x-auto">
            <div className="relative min-w-[800px]">
              ...
              {/* Tracks */}
              {tracks.map((track) => (
                <div key={track.id} className="h-20 border-b  relative bg-gray-850">
                    ...
                    {track.clips.map((clip) => (
                      <div
                        ...
                        style={{
                          ...
                          borderColor: selectedScene === Number.parseInt(clip.id.slice(-1)) ? "#fbbf24" : clip.color,
                        }}
                        onClick={() => onSceneSelect(Number.parseInt(clip.id.slice(-1)))}
                      >
                        {track.type === "video" ? (
